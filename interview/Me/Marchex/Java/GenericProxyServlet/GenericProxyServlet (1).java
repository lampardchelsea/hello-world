...
public class GenericProxyServlet extends HttpServlet {
	...
	/** The thread pool to use for submitting requests to services */
	...
	/** The pq handler. */
	private PriorityQueueHandler pqHandler;
	...
	public void init() throws ServletException {
		pqHandler = new PriorityQueueHandler(remoteCapacity, this);
		viaHost = viaHost();
	}
	...
	protected void service(final HttpServletRequest request, final HttpServletResponse response)
		...
		if (newUriWithStatus.getValue().intValue() == 401 || newUriWithStatus.getValue().intValue() == 403) {
			processError(request, response, newUriWithStatus.getValue().intValue());
			return;
		} else if (newUriWithStatus.getValue().intValue() == 999) {
		...
		if (request.getMethod().equals(HttpMethod.POST.asString())) {
			String requestId = ProviderHandler.locateProvider(pqHandler.getRemoteCapacity(), request)
					.loggingInfoToTable(pqHandler, request);
			Map<String, String> queryMap = ProxyUtil.getQueryMap(request.getQueryString());
			handleIncomingRequest(request, response, queryMap, requestId, pqHandler, rewrittenURI);

			/*
			 * if (proxyRequestPair.getValue()) {
			 * proxyRequestPair.getKey().content(proxyRequestContent(
			 * proxyRequestPair.getKey(), request)); }
			 *
			 * Request customProxyRequest =
			 * customizeProxyRequest(proxyRequestPair.getKey(), request);
			 * customProxyRequest.timeout(getTimeout(), TimeUnit.MILLISECONDS);
			 *
			 * // Future<Integer> futureCall = getFutureCall(customProxyRequest,
			...
			forwardRequestAsyncCall(request, response, rewrittenURI);
		}

	}

	protected void handleIncomingRequest(HttpServletRequest request, HttpServletResponse response,
			Map<String, String> queryMap, String requestId, PriorityQueueHandler pqHandler, URI rewrittenURI)
			throws ServletException, IOException {

	}

	protected boolean queueProcessing(PriorityRequest pr, boolean flag) {
		return false;
	}
	 ...
	 */
	public Pair<Request, Boolean> getProxyRequest(HttpServletRequest request, URI uri) {
		...
		if (method.equalsIgnoreCase("POST")) {
			proxyRequest = proxyHttpClient.POST(uri).version(HttpVersion.fromString(request.getProtocol()));
		} else {
			proxyRequest = proxyHttpClient.newRequest(uri).method(request.getMethod())
	 ...
	 */
	public Timestamp forwardRequestAsyncCall(HttpServletRequest request, HttpServletResponse response, URI newUri
			// , Request proxyRequest, boolean hasContent
														) throws ServletException, IOException {
		final Pair<Request, Boolean> proxyRequestPair = getProxyRequest(request, newUri);
		final AsyncContext asyncContext = request.startAsync();
		// timeout the proxy request, not the continuation
		asyncContext.setTimeout(10000);
		request.setAttribute(ASYNC_CONTEXT, asyncContext);

		if (proxyRequestPair.getValue().booleanValue()) {
			// System.out.println(SoapParser.extractSoapEnvelope(request));
			// System.out.println(SoapParser.extractSoapEnvelope(proxyRequest));
			proxyRequestPair.getKey().content(proxyRequestContent(proxyRequestPair.getKey(), request));
		}

		Request customProxyRequest = customizeProxyRequest(proxyRequestPair.getKey(), request);
		customProxyRequest.timeout(getTimeout(), TimeUnit.MILLISECONDS);

		/** The flag of complete response. */
		boolean isResponseComplete = false;
		ProxyResponseListener listener = new ProxyResponseListener(request, response, isResponseComplete);
		proxyRequestPair.getKey().send(listener);

		System.out.println("Response status code BEFORE response LOCK: " + response.getStatus());
		try {
			Thread.sleep(1000);
			/*Object responseLock = listener.getResponseLock();
			synchronized (responseLock) {
				System.out.println("Inside ForwardAsync call response Lock.");
				while (!listener.isResponseComplete) {
					responseLock.wait();
				}
			}*/
		} catch (InterruptedException e) {
			System.out.println("Exception in Response LOCK!");
			e.printStackTrace();
		}

		System.out.println("Response status code AFTER response LOCK: " + response.getStatus());
		return listener.endTime;
	}
	...
	protected ContentProvider proxyRequestContent(Request proxyRequest, final HttpServletRequest request)
		...
		return new InputStreamContentProvider(request.getInputStream()) {
			...
			protected ByteBuffer onRead(byte[] buffer, int offset, int length) {
				// log.info((String.format("%d proxying content to upstream: %d
				// bytes", getRequestId(request), length)));
	...
	/**
	 * The listener interface for receiving proxyResponse events. The class that
	 * is interested in processing a proxyResponse event implements this
	 * interface, and the object created with that class is registered with a
	 * component using the component's <code>addProxyResponseListener
	 * <code> method. When the proxyResponse event occurs, that object's
	...
	private class ProxyResponseListener extends Response.Listener.Adapter {
		 ...
		 *
		 * @see
		 * org.eclipse.jetty.client.api.Response.Listener.Adapter#onBegin(org.
		 * eclipse.jetty.client.api.Response)
		 */
		@Override
		 ...
		 *
		 * @see
		 * org.eclipse.jetty.client.api.Response.Listener.Adapter#onHeaders(org.
		 * eclipse.jetty.client.api.Response)
		 */
		@Override
		 ...
		 *
		 * @see
		 * org.eclipse.jetty.client.api.Response.Listener.Adapter#onContent(org.
		 * eclipse.jetty.client.api.Response, java.nio.ByteBuffer)
		 */
		@Override
		public void onContent(Response proxyResponse, ByteBuffer content) {
			// _log.info("in onContent of ProxyResponseListener -- length: " +
			// content.remaining());
			byte[] buffer;
			int offset;
			...
			try {
				...
				log.log(Level.SEVERE, "!!!!!!!!!!!! About to abort request: " + getRequestId(request) + " >>> "
		 ...
		 *
		 * @see
		 * org.eclipse.jetty.client.api.Response.Listener.Adapter#onSuccess(org.
		 * eclipse.jetty.client.api.Response)
		 */
		@Override
		public void onSuccess(Response proxyResponse) {
			...
			Object responseLock = getResponseLock();
			synchronized (responseLock) {
		 ...
		 *
		 * @see
		 * org.eclipse.jetty.client.api.Response.Listener.Adapter#onFailure(org.
		 * eclipse.jetty.client.api.Response, java.lang.Throwable)
		 */
		@Override
		 ...
		 *
		 * @see
		 * org.eclipse.jetty.client.api.Response.Listener.Adapter#onComplete(org
		 * .eclipse.jetty.client.api.Result)
		 */
		@Override
