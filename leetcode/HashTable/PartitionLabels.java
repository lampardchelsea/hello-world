...
Solution 1: Greedy + Hash Table + 2 Pass O(n) space (30 min)
class Solution { 
    public List<Integer> partitionLabels(String s) { 
        List<Integer> result = new ArrayList<Integer>(); 
        Map<Character, Integer> map = new HashMap<Character, Integer>(); 
...
Solution 2: Greedy + Hash Table + 2 Pass O(1) space (30 min)
...
Still, confuse. Don't worry just walk with me >>First create an HashMap of Character & Integer 
[where integer tell us maximum imapct of a character at this index] & after that fill it: 
