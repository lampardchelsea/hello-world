...
["ba", "a", "aaa"], the Trie structure will be as follows:
        root (f)
           | 'a'
          n1 (t)
     ------------
 'b' |          | 'a'
    n2 (t)    n3 (f)
                | 'a'
              n4 (t)
The letter in parentheses indicates the value of isWord for each node: f ==> false and t ==> true. The letter beside each vertical line denotes the index into the 
next array of the corresponding node. For example, for the first vertical line, 'a' means root.next[0] is not null. Similarly 'b' means n1.next[1] is not null, and so on.
...
Both building and searching the Trie structure take O(n * k^2), (n: total number of words; k: average length of each word) which sets the total time complexity of the solution. 
...
class Solution {
    ...
    public void search(String[] words, int i, TrieNode root, List<List<Integer>> result) {
        for(int j = 0; j < words[i].length(); j++) {
            // (1) root.index >= 0 --> finished construct of current word as an existing word of in given words, locate at index, not tag index as -1 anymore
