...
1.The hundreds digit represents the depth D of this node, 1 <= D <= 4.2.The tens digit represents the position P of this node in the level it belongs to, 1 <= P <= 8. The position is the same as that in a full binary tree.3.The units digit represents the value V of this node, 0 <= V <= 9.
...
---
Attempt 1: 2022-11-07

Solution 1: Preorder recursive traversal + HashMap (60min)
```
public class Solution { 
    /** 
     * @param nums: the list 
    ...
    private void helper(Map<Integer, Integer> map, int key, int preSum) { 
        int level = key / 10; 
        int pos = key % 10; 
        int left = (level + 1) * 10 + pos * 2 - 1; 
        int right = (level + 1) * 10 + pos * 2; 
        ...
        if(map.containsKey(left)) { 
            helper(map, left, curSum); 
        } 
        if(map.containsKey(right)) { 
            helper(map, right, curSum); 
        } 
    } 
}
...
Solution 2:  Level order iterative traversal  + HashMap (60min)
```
public class Solution { 
    /** 
     * @param nums: the list 
    ...
    public int pathSumIV(int[] nums) { 
        ...
        while(!q.isEmpty()) { 
            ...
            int left = (level + 1) * 10 + pos * 2 - 1; 
            int right = (level + 1) * 10 + pos * 2; 
            // Find a leaf node, valid path(s) get, add to the result 
            if(!map.containsKey(left) && !map.containsKey(right)) { 
            ...
            if(map.containsKey(left)) { 
                q.offer(left); 
                map.put(left, map.get(left) + map.get(key)); 
            } 
            // Update right node's value by adding its parent root value 
            if(map.containsKey(right)) { 
                q.offer(right); 
                map.put(right, map.get(right) + map.get(key)); 
            } 
        } 
        return totalSum; 
    } 
...
Refer to
https://www.cnblogs.com/grandyang/p/7570954.html
下面这种方法是迭代的形式，我们使用的层序遍历，与先序遍历不同的是，我们不能维护一个当前路径之和的变量，这样会重复计算结点值，而是在遍历每一层的结点时，加上其父结点的值，如果某一个结点没有子结点了，才将累加起来的结点值加到结果res中
