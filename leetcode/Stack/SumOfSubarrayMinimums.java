...
3 4 6
```
How much the element 3 contributes to the final answer? It is 3*(4*3).What is the final answer? Denote by left[i] the distance between element A[i] and its PLE. Denote by right[i] the distance between element A[i] and its NLE.
...
Method: Set strict less and non-strict less(less than or equal to) for finding NLE and PLE respectively. The order doesn't matter.
For example, the above code for finding NLE is strict less, while PLE is actually non-strict less. Remark: Although in both loop conditions the signs are set as >, for NLE, we make records inside the loop, while for PLE, records are done outside the loop.
...
Mathematically the total number of subarrays containing 2 can be obtained as follows: Let L = total number of left subarrays = 5Let M = total number of middle subarrays = 1Let R = total number of right subarrays = 5N = Total number of subarrays containing element in middle subarray N = L * M * R = 5 * 1 * 5 = 25
...
The value 1 at index 8 is about to pop value 2 at index 4 off the stack. Reference index i = 4 = stack.pop( )i1 = Stack[-1] = 0i2 = 8L = i – i1 = 4 – 0 = 4R = i2 – i = 8 – 4 = 4N = 4 * 4 = 16
