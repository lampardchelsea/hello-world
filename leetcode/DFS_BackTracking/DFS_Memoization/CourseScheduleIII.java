...
In order to solve the given problem, we make use of a recursive function schedule(courses, i, time) which returns the maximum number of courses that can be taken starting from the ith course(starting from 0), given the time aleady consumed by the other courses is time, i.e. the current time is time, given a courses array as the schedule.
Now, in each function call to schedule(courses, i, time), we try to include the current course in the taken courses. But, this can be done only if time+durationi < end_dayi​. Here, durationi refers to the duration of the ith course and end_dayi refers to the end day of the ith course.
...
Further, for every current course, we also leave the current course, and find the number of courses that can be taken thereof. Now, we need not update the time, but we need to update the courses' index. Thus, we make the function call, schedule(courses, i + 1, time). Let's say, we store the count obtained in not_taken variable.
While returning the number of courses at the end of each function call, we return the maximum value out of taken and not_taken.
