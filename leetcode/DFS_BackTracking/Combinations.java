...
T(n)  = nT(n-1)         
      = n*[(n-1)*T(n-2)]        
      = (n)*(n-1)*(n-2)*(n-3)......(n-k)*T(n-k-1)when recursive reach the kth level and try to do T(n-k-1) 
curr.size()==k is true, because every level curr add one numberT(n-k-1) will simply 
ans.add(new ArrayList(curr)) and returns which means T(n-k-1) = 1
so
...
--------------------------------------------------------------------------------
根据Backtracking模式下的DFS没法直接转变成DP的情况，我们采用Divide and Conquer技术下的DFS来做DP演化 
Refer to
...
dp[0][0] must be {{}} even it satisfy both k = 0 and k = n, but if we see two base conditions 
in Divide and Conquer recursion, we will see for k = 0, dp[0][0] will be {{}}, for k = n, dp[0][0] 
will also be {{}} since loop of i to fill the row start from i = 1 till i <= k(=0), which means 
