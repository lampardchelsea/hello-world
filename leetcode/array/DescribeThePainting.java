...
Solution 1: Sweep Line + Sorting (Treemap) + Two Pointers (30 min)
```
class Solution {
...
6. Build Resulting Segments: When the cumulative color value changes from one non-zero value to another, we add a new segment to our output result. After processing the sorted list, the output might look like this:
...
class Solution {
    public List<List<Long>> splitPainting(int[][] segments) {
        // A TreeMap stores unique keys in sorted order, along with their corresponding values.
        // Here, keys track the position on the canvas and values track paint color changes.
        TreeMap<Integer, Long> deltaMap = new TreeMap<>();

        // Process each paint segment and update the deltas for start and end points
        for (int[] segment : segments) {
            int start = segment[0], end = segment[1];
            long color = segment[2];
            // Increase color value at the start point
            deltaMap.put(start, deltaMap.getOrDefault(start, 0L) + color);
            // Decrease color value at the end point
            deltaMap.put(end, deltaMap.getOrDefault(end, 0L) - color);
        }

        // Prepare a list to store the result segments
        List<List<Long>> result = new ArrayList<>();
        long previousPosition = 0, currentPosition = 0;
        long currentColorSum = 0;

        // Go through each entry in the map to create the painted segments
        for (Map.Entry<Integer, Long> entry : deltaMap.entrySet()) {
            // If it's the first key, initialize the previous position.
            if (Objects.equals(entry.getKey(), deltaMap.firstKey())) {
                previousPosition = entry.getKey();
            } else {
                currentPosition = entry.getKey();
                // If the color sum is positive, add the segment to the result list.
                if (currentColorSum > 0) {
                    result.add(Arrays.asList(previousPosition, currentPosition, currentColorSum));
                }
                // Update the previous position for the next iteration
                previousPosition = currentPosition;
            }
            // Update the color sum with the delta value
            currentColorSum += entry.getValue();
        }
        return result;
    }
}
...
Combining all of these, the overall time complexity of the function is determined by the most time-consuming operation, which in this case is the sorting step. So, the overall time complexity of the code is O(nlogn).
