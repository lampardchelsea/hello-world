 ...
 * Well, some boundary conditions need to be handled. The boundary conditions happen on the topmost 
 * row (S[i - 1][j] does not exist) and the leftmost column (S[i][j - 1] does not exist). 
...
public class Solution {
    public int minPathSum(int[][] grid) {
        ...
        int n = grid[0].length;
        //int[][] dp = new int[m][n];
        // The boundary case for first column
        for(int i = 1; i < m; i++) {
            grid[i][0] = grid[i - 1][0] + grid[i][0];
        }
        // The boundary case for first row
        for(int j = 1; j < n; j++) {
