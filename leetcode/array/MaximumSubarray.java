...
https://leetcode.com/problems/maximum-subarray/solutions/20452/c-dp-and-divide-and-conquer/
...
At each index, we update dp[1][i] as max between either only choosing current element - nums[i] or extending from previous subarray and choosing current element as well - dp[1][i-1] + nums[i]Similarly, dp[0][i] can be updated as max between maximum sum subarray found till last index - dp[0][i-1] or max subarray sum found ending at current index dp[1][i].
```
class Solution {
public:
    int maxSubArray(vector<int>& nums) {
