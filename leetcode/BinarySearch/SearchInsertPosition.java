...
---
Solution 2: 60min, template based on while(lo < hi), refer L34.Find First and Last Position of Element in Sorted Array, too long because of trick on setting hi = len rather than hi = len - 1
```
class Solution {
    // Convert problem into find the most recent value position smaller
    // or equal to target
...
---
Solution 3: 30min, no template based while(lo < hi), intuitive solution
```
class Solution {
    // Convert problem into find the most recent value position smaller
    // or equal to target
...
1 	5 	7 	9
	      ...
	      result
```
6. Then how to determine the while loop condition, l < r, l <= r or l < r - 1? The answer is that we analyze and conclude with the one which will not cause deal while loop in our code while we are moving our left and right pointers.l = m + 1 and r = m
