...
翻译如下：有两个有序数组 nums1和 nums2，长度分别为m和n，在最大时间复杂度为 O(log(m+n))的要求下找到将两个数组合并成的有序数组 nums3的中位数。
...
不难发现， partitionA和 partitionB必须满足以下关系。
...
因此，只要确定 partitionA或 partitionB 中任意一个值，我们就能推出另外一个值，并且求得中位数!
